...
- Banco: SQLite para dados do app e logs de desenvolvimento
...
│   ├── models.py            # Modelos desativados (histórico em sessão)
...
- `POST /send/` → `SendMessageView`: recebe `{"message": "..."}`, persiste na sessão, chama IA, retorna `{"reply": "..."}`
- `POST /recommend/` → `RecommendView`: usa histórico da sessão, retorna `{"products": [...]}`

### `chat/models.py`

- Modelos `Conversation` e `Message` estão desativados (comentados)
- O histórico ativo fica em `request.session["chat_history"]`
- O estado da conversa é limitado às últimas mensagens em sessão
...
LLM_PROVIDER=openai       # Valor padrão atual em settings.py
...
**Nunca hardcodar chaves no código-fonte. Use `.env.example` como base local e não commite chaves reais no `.env`.**
...
- Histórico de conversa mantido em sessão via cookies assinados
